...
class Server():
    def __init__(self, port=8000, coap_port=5683, login=None, password=None, passwdfile=None, configfile=None, scriptfile=None):
        ...
        manager.addDeviceInstance("GPIO", self.gpio, [])	#output   "GPIO - Native mapped to REST API /GPIO"
        ...
        self.gpio.addSetups(config.items("GPIO"))		#/etc/webiopi/config  GPIO
        self.gpio.addResets(config.items("~GPIO"))
        self.gpio.setup()
        
        devices = config.items("DEVICES")	 #/etc/webiopi/config  DEVICES
        for (name, params) in devices:		#temp1 = TMP102 slave:0x49
            values = params.split(" ")	
            driver = values[0];
            args = {}
            i = 1
        ...
        scripts = config.items("SCRIPTS")		#/etc/webiopi/config  SCRIPTS
        for (name, source) in scripts:
            loader.loadScript(name, source, self.restHandler)
        
        self.restHandler.device_mapping = config.getboolean("REST", "device-mapping", True)	 #/etc/webiopi/config  REST device-mapping = false
        self.gpio.post_value = config.getboolean("REST", "gpio-post-value", True)			#gpio-post-value = false
        ...
        http_port = config.getint("HTTP", "port", port)						#/etc/webiopi/config  HTTP
        ...
        coap_port = config.getint("COAP", "port", coap_port)				#/etc/webiopi/config  COAP
        coap_enabled = config.getboolean("COAP", "enabled", coap_port > 0)
        coap_multicast = config.getboolean("COAP", "multicast", coap_enabled)

        routes = config.items("ROUTES")										#/etc/webiopi/config  ROUTES
